...
R = [0.15, 0.15, 0.15, 0.15, 0.15, 0.15, 0.15]
...
def propagate():
    ...
    for i in range(0,Labeled_tlm.shape[0]):
        for j in range(0,Labeled_tlm.shape[1]):
            for k in range(0,Labeled_tlm.shape[2]):
...
def overal_pressure():
    ...
    IN_temp = IN#[2:-2,2:-2,2:-2]
    IE_temp = IE#[2:-2,2:-2,2:-2]
    IS_temp = IS#[2:-2,2:-2,2:-2]
    IW_temp = IW#[2:-2,2:-2,2:-2]
    IU_temp = IU#[2:-2,2:-2,2:-2]
    ID_temp = ID#[2:-2,2:-2,2:-2]
...
def meas_pressure():
    ...
    for item in pressure_grid:
        ...
        mic_overal.append(np.sqrt(np.sum(np.power(np.array(item),2)) / ((nx-2)*(ny-2)*(nz-2))))
